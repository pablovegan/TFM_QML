...
class QML_optimizer(QML_model):
    def __init__(self, f, x, cost_fun, probability = None,
        ...
        super().__init__(f, x, cost_fun, probability)
    ...
    def blackbox_minimizer(self, φ_init):
    ...
    def train_perceptron(self,
        ...
        if self.opt_method == 'ADAM':
            φ = self.adam(φ_init, **self.method_params)
            result = 0  # resultado a 0 por defecto en este método
        else:
            result = self.blackbox_minimizer(φ_init)
            φ = result.x
