...
class Cost:
    ...
    def _layer(self, θ: np.ndarray, w: float) -> tuple:
        ...
        return np.einsum('mn, np, pqg -> gmq', Rz, Ry, Rx) # move the x axis to first position

    def _encoding(self, θ: np.ndarray, w: np.ndarray):
        """
    ...
    def _der_amp_encoding(self, θ: np.ndarray, w: np.ndarray):
        ...
        for i in range(layers):  
            ...
            D[i,...] = np.einsum('jgmn, gnp -> jgmp', DUi, U)  # j is each of the derivatives
    ...
    def grad_mse(self, θ: np.ndarray, w: np.ndarray):
        ...
        if self.prob:
            ...
            fn_approx, grad_enc = self.der_amp_encoding(θ, w)
    ...
    def grad_rmse(self, θ: np.ndarray, w: np.ndarray):
        ...
        fn_diff = fn_approx - self.fn
        der_C = 2/(np.sqrt(self.x_size) * np.sqrt(np.sum(np.abs(fn_diff)**2) + 1e-9)) * ders
    ...
    def grad(self, θ: np.ndarray, w: np.ndarray, return_cost = False):
        ...
        if self.probability:
            ...
            if self.cost_fun == 'MSE':
                der_C = 4/self.x_size * np.array([[np.dot(E.real, np.real(np.conj(U)*ders[i,j,:])) for i in range(layers)] for j in range(4)])
            else:
                der_C = 2/(np.sqrt(self.x_size)*np.sqrt(np.sum(np.abs(E)**2)+1e-9)) * np.array([[np.dot(E, np.real(np.conj(U)*ders[i,j,:]))
                                                                                                 for i in range(layers)] for j in range(4)])
        else:
            E = U - self.fn   # error in approximation
            if self.cost_fun == 'MSE':
                der_C = 2/self.x_size * np.array([[np.real(np.dot(np.conj(E), ders[i,j,:])) for i in range(layers)] for j in range(4)])
            else:
                der_C = 1/(np.sqrt(self.G)*np.sqrt(np.sum(np.abs(E)**2)+1e-9)) * np.array([[np.real(np.dot(np.conj(E), ders[i,j,:]))
                                                                                            for i in range(layers)] for j in range(4)])
        # devolvemos un array con la misma estructura que ϕ = [w, θ_0, θ_1, θ_2]
        if return_cost:
            if self.prob:
                return der_C.flatten(), np.mean((np.abs(U*np.conjugate(U)) - self.fn)**2)
            else:
